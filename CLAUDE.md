...
- **`bookCard()` must generate language-aware hrefs.** For books that have a `translated` array and `volumesPublished`, the link must be prefixed with the current language when `lang !== 'ar'`. The pattern: `lang + '/' + b.href + b.volumesPublished[0] + '/1/'`. Without this, English/Farsi/Urdu users clicking a book card land on the Arabic URL — the bug is invisible until you test with a non-Arabic language active.
- **`applyLang()` must rewrite `.vols a.vol` links.** Book index pages (e.g. `bihar/index.html`) have volume links like `href="1/"`. These must be rewritten to `ROOT + '/' + lang + '/' + slug + '/' + v + '/1/'` when a non-Arabic language is active. The slug is inferred from `location.pathname`. The original volume number is cached in a `data-vol` attribute on first call.
- **Book index pages have no `#page-meta` with `data-alt-*`.** This means the language switcher on `bihar/index.html` cannot navigate to a translated URL — it only changes UI strings. Language-aware navigation from those pages relies entirely on the vol-link rewriting in `applyLang()` described above.
